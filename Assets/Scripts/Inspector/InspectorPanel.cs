		...
		public void Init(LevelEventInfo levelEventInfo)
        {
            ...
            using (var enumerator = propertiesInfo.Keys.GetEnumerator())
            {
                while (enumerator.MoveNext())
                {
                    ...
                    var dontRenderKeys = (List<string>)typeof(PropertiesPanel)
                        .GetField("dontRenderKeys", AccessTools.all).GetValue(null);
                    if (dontRenderKeys.Contains(propertyKey)) continue;

                    PropertyInfo propertyInfo = propertiesInfo[propertyKey];
                    if (propertyInfo.controlType == ControlType.Hidden || propertyInfo.pro) continue;

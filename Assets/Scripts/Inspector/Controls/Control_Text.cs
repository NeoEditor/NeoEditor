    ...
    public class Control_Text : ControlBase
    {
		...
		public override void Setup(bool addListener)
		{
					...
					using (new SaveStateScope(editor, false, true, false))
					{
						...
						switch (type)
						{
							...
							default:
								...
								{
									Tuple<int, TileRelativeTo> tuple = selectedEvent[name] as Tuple<int, TileRelativeTo>;
			...
			if (!string.IsNullOrEmpty(propertyInfo.unit))
			{
				...
				unit.text = RDString.Get("editor.unit." + propertyInfo.unit, null, LangSection.Translations);
			}
		}

...
namespace NeoEditor.Inspector.Controls
{
    public class Control_File : ControlBase
    {
		...
		public override List<Selectable> selectables 
			=> new List<Selectable> { inputField, button };
		...
		private void BrowseFile()
		{
			...
			{
				newFilename = RDEditorUtils.ShowFileSelectorForAudio(RDString.Get("editor.dialog.selectSound", null, LangSection.Translations), -1L);
			}
			else if (fileType == FileType.Image)
			{
				newFilename = RDEditorUtils.ShowFileSelectorForImage(RDString.Get("editor.dialog.selectImage", null, LangSection.Translations), -1L);
			}
			else if (fileType == FileType.Video)
			{
				newFilename = RDEditorUtils.ShowFileSelectorForVideo(RDString.Get("editor.dialog.selectVideo", null, LangSection.Translations), -1L);
			}
		...
		private void ProcessFile(string newFilename, FileType fileType)
		{
			...
			if (fileType != FileType.Audio)
			{
						...
						if (selectedEvent.eventType == LevelEventType.MoveDecorations)
						{
							...
							if (selectedEvent.data.TryGetValue("decorationImage", out obj))
							{
								...
								if (text != null && !string.IsNullOrEmpty(text))
								{
									...
									ADOBase.customLevel.imgHolder.AddSprite(text, filePath, out loadResult);
									//editor.UpdateImageLoadResult(text, loadResult);
								}
							}
						}
						else if (propertyInfo.affectsFloors || selectedEvent.eventType == LevelEventType.ColorTrack)
						{
				...
				if (fileType == FileType.Video)
				{
					...
					videoBG.url = Path.Combine(Path.GetDirectoryName(ADOBase.levelPath), editor.levelData.miscSettings.data["bgVideo"].ToString());
					videoBG.Stop();
					videoBG.Prepare();
					base.ToggleOthersEnabled();
				}

    ...
    public class EditorControls : MonoBehaviour
    {
        ...
        void Update()
        {
			...
			if (conductor.song.clip)
			{
				...
				time.text = $"{now.Minutes}:{now.Seconds.ToString("00")}.{now.Milliseconds.ToString("000")}\t({scrController.instance.currentSeqID}/{NeoEditor.Instance.floors.Count - 1})";
        ...
        void ShiftSpeed(int direction)
        {
			...
			int mul = (Input.GetKey(KeyCode.LeftShift) || Input.GetKey(KeyCode.RightShift)) ? 1 : 10;
			mul *= direction;
